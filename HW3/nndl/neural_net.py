...
class TwoLayerNet(object):
  ...
  def loss(self, X, y=None, reg=0.0):
    ...
    num_train = X.shape[0]
    selector = np.arange(num_train), y
    
    temp_scores = scores
    temp_scores -= np.max(temp_scores)
    loss = np.sum(np.log(np.sum(np.exp(temp_scores).T, axis=0)) - temp_scores[selector]) / num_train
    ...
    dldh2 = prob.sum(axis=0)
    
    grads['b2'] = dldh2
    grads['W2'] = h1.T.dot(prob).T + reg * W2
    
    dldh1 = prob.dot(W2)
    dlda = dldh1 * (a > 0)
    
    grads['b1'] = dlda.sum(axis=0)
    grads['W1'] = X.T.dot(dlda).T + reg * W1
