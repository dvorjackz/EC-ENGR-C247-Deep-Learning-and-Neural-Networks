...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        sample_var = (np.sum((x - sample_mean)**2, axis=0)) / num_train
...
def batchnorm_backward(dout, cache):
    ...
    dldx_hat = dldy * gamma
    dlda = 1 / np.sqrt(sample_var + eps) * dldx_hat
    dldb = (x - sample_mean) * dldx_hat
    dldc = -1 / (sample_var + eps) * dldb
    dlde = -1 / (2 * (sample_var + eps)**(3/2)) * dldb
    dldv = np.sum(dlde, axis=0)
    dldmu = -1 / np.sqrt(sample_var + eps) * np.sum(dldx_hat, axis=0) - dldv * 2 * np.mean(x - sample_mean, axis=0)
    dvdx = 2 / num_train * (x - sample_mean)
    dldx = dlda + dvdx * dldv + 1 / num_train * dldmu
