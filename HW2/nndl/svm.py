...
class SVM(object):
  ...
  def loss(self, X, y):
    ...
    # ================================================================ #
      for j in np.arange(num_classes):
        if y[i] != j:
            loss += max(0, 1 + self.W[j].dot(X[i]) - self.W[y[i]].dot(X[i]))
  ...
  def loss_and_grad(self, X, y):
    ...
    # ================================================================ #
      for j in np.arange(num_classes):
        if y[i] != j:
          zj = 1 + self.W[j].dot(X[i]) - self.W[y[i]].dot(X[i])
          loss += max(0, zj)
          grad[j] += 0 if zj <= 0 else X[i]
          grad[y[i]] += 0 if zj <= 0 else -X[i]
  ...
  def fast_loss_and_grad(self, X, y):
    ...
    losses = np.ones_like(scores) + scores - scores[np.arange(num_train), y].reshape(scores.shape[0], 1)
    losses[losses < 0] = 0
    ...
    selector[y, np.arange(num_train)] = np.sum(selector, axis=0) * (-1)
    grad = selector.dot(X) / num_train
