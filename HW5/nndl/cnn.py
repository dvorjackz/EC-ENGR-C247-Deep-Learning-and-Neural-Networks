...
class ThreeLayerConvNet(object):
  ...
  def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
    ...
    stride = 1
    pad = (filter_size - 1) // 2
    h_conv, w_conv = (h - filter_size + 2*pad) // stride + 1, (w - filter_size + 2*pad) // stride + 1
  ...
  def loss(self, X, y=None):
    ...
    loss += 0.5 * self.reg * (sum([np.sum(self.params["W" + str(i)]**2) for i in range(1, 4)]))
    
    dx_aff, grads["W3"], grads["b3"] = affine_backward(dout, aff_cache)
    dx_relu, grads["W2"], grads["b2"] = affine_relu_backward(dx_aff, relu_cache)
    # resuscitate the feature map axes from the flattened
    dx_resuscitated = dx_relu.reshape(conv_out.shape)
    dx_conv, grads["W1"], grads["b1"] = conv_relu_pool_backward(dx_resuscitated, conv_cache)
